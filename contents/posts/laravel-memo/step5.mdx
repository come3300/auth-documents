...
![完成イメージ（変更前）](/images/laravel-memo/before.png)
...
<InfoBox type="info" title="Blade のポイント">
...
<InfoBox type="info" title="レイアウトのポイント">
- `@yield('content')` に各画面の `@section('content')` が入る
- `<meta name="csrf-token"...>` で CSRF 対策トークンを埋め込む（Laravel 標準）
- `{{ asset('css/app.css') }}` / `{{ asset('js/app.js') }}` は `public/css`, `public/js` のビルド済みを参照（`npm run dev` / `npm run build` 等で出力）
</InfoBox>
...
![完成後](/images/laravel-memo/after.png)

<Note type="tip">
変更が反映されない場合は、キャッシュをクリアしてください。
</Note>
...
<InfoBox type="warn" title="画像が表示されない">
`public/images/animal_stand_zou.png` が存在するか、パスが合っているかを確認してください。
</InfoBox>

<InfoBox type="warn" title="スタイルが当たらない">
`public/css/app.css` のビルド済みファイルがあるか、`npm install && npm run dev` を実行済みか確認してください。
</InfoBox>

<InfoBox type="warn" title="アプリ名が変わらない">
`.env` の `APP_NAME` 修正後に `php artisan config:clear` を実行してください。
</InfoBox>
