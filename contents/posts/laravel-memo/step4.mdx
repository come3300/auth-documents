...
![Login画面](/images/laravel-memo/login.png)

![Register画面](/images/laravel-memo/register.png)
...
![ビルド前](/images/laravel-memo/before_build.png)

<Note type="warn">
CSSがまだビルドされていないため、見た目は崩れています。次に、フロントエンド資産をビルドして整えます。
</Note>
...
<InfoBox type="info" title="処理の流れ">
- `npm install` → Node.js関連ライブラリのインストール
- `npm run dev` → `webpack.mix.js`をもとに`public/css`と`public/js`へビルド
</InfoBox>
...
![ログイン画面（ビルド後）](/images/laravel-memo/login.png)

綺麗にスタイルが当たったログイン画面が表示されました。

右上の **Register** をクリックすると、ユーザー登録画面が表示されます。

![ユーザー登録画面](/images/laravel-memo/register.png)
